...
def main():
    parser = argparse.ArgumentParser(description='Evaluate VFH+ (FoV-limited)')
    ...
    results = run_evaluation(method='vfh_plus_fov', num_runs=args.runs, cfg=cfg, base_seed=args.seed)
    summary = summarize_results(results)

    os.makedirs(os.path.join(os.path.dirname(__file__), args.outdir), exist_ok=True)
    csv_path = os.path.join(os.path.dirname(__file__), args.outdir, 'vfh_plus_fov_results.csv')
