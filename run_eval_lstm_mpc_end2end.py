...
def main():
    parser = argparse.ArgumentParser(description='Evaluate End-to-End LSTM waypoint proposal + MPC tracking')
    ...
    results = run_evaluation(method='lstm_mpc_end2end', num_runs=args.runs, cfg=cfg, dtqn_checkpoint=ckpt_path, base_seed=args.seed)
    summary = summarize_results(results)

    os.makedirs(os.path.join(os.path.dirname(__file__), args.outdir), exist_ok=True)
    csv_path = os.path.join(os.path.dirname(__file__), args.outdir, 'lstm_mpc_end2end_results.csv')
